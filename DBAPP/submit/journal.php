...
WHERE 
    ...
    sc1.sc_rank<sc2.sc_rank AND sc3.sc_rank<sc4.sc_rank AND
	sc1.sc_station_id=st1.st_id AND sc2.sc_station_id=st2.st_id AND
	sc3.sc_station_id=st3.st_id AND sc4.sc_station_id=st4.st_id AND
	st1.st_city<>st2.st_city AND st3.st_city<>st4.st_city;
';
...
<html>
	...
	<body>
		...
		<a href=back.php?dcity=<?php echo $acity?>&acity=<?php echo $dcity?>&ddate=<?php echo $ddate?>>
		返程订票</a><br>
		直达列车：<br>
		<table border="1">
			<tr>
				<td>车次</td><td>出发站</td><td>到达站</td><td>出发日期</td><td>出发时间</td><td>总耗时</td><td>座位类型</td><td>票价</td><td>余票</td>
	<?php
		while ($line = pg_fetch_array($result1, null, PGSQL_ASSOC)) {
			...
			foreach($line as $key=>$value)
			{
				if($key=='ticket_left')
				{
					$str = urlencode(serialize($line));
					echo '<td><a href=book.php?line=' . $str . '>' . $value . '</a></td>';
				}
				else
					echo '<td>' . $value . '</td>';
			}
