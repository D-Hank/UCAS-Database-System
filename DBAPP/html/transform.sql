...
WHERE mstation1=mstation2 AND train1<>train2 AND
...
CREATE table good_transfer_train_same_station(
    ...
    mstation varchar(20) NOT NULL,
    dstation_id integer NOT NULL,
    astation_id integer NOT NULL,
    mstation_id integer NOT NULL,
...
SELECT DISTINCT
    row_number() over(order by train1) AS id,
	ts.train1 AS train1, 
	ts.train2 AS train2, 
	...
	(CASE
    	WHEN sc1.sc_arrive_time-sc1.sc_depart_time>='00:00'
    ...
    (CASE
    	WHEN sc3.sc_arrive_time-sc3.sc_depart_time>='00:00'
     	THEN sc4.sc_total_time-sc3.sc_total_time
     		 -(sc3.sc_arrive_time-sc3.sc_depart_time)
     	WHEN sc3.sc_arrive_time-sc3.sc_depart_time<'00:00'
     	THEN sc4.sc_total_time-sc3.sc_total_time
     		 -(sc3.sc_arrive_time-sc3.sc_depart_time+INTERVAL '24:00:00')
     END
    ) AS total_time2,
    (CASE
...
SELECT
    ...
    gt.total_time2 + gt.transfer_time AS total_time2,
...
WHERE 
    ...
    sc2.sc_station_id=gt.mstation_id AND sc2.sc_train_num=gt.train1 AND
	sc3.sc_station_id=gt.mstation_id AND sc3.sc_train_num=gt.train2 AND
    sc4.sc_station_id=gt.astation_id AND sc4.sc_train_num=gt.train2 AND
    gt.train1=pd.train1 AND gt.dstation=pd.dstation1 AND 
    gt.train2=pd.train2 AND gt.mstation=pd.dstation2;
...
FROM
    transfer_result1
GROUP BY
    id, train_num, dstation, astation, depart_time, total_time, seat_type, price, ddate;
...
SELECT 
    ...
    tr2.price AS price2,
    tr2.ticket_left AS ticket_left
FROM 
...
SELECT
    ...
    INPUT.ddate AS ddate,
    INPUT.depart_time AS depart_time,
    dstation AS dstation,
    date(INPUT.ddate+INPUT.depart_time+INPUT.total_time) AS adate,
    time(INPUT.ddate+INPUT.depart_time+INPUT.total_time) AS atime,
    astation AS astation,
    seat_type AS seat_type,
    INPUT.price AS price
FROM
    final_result
WHERE
    train_num=INPUT.train_num AND dstation=INPUT.dstation AND
    astation=INPUT.astation AND seat_type=INPUT.seat_type;
